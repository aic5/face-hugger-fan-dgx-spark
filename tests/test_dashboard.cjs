...
(async()=>{
  ...
  const curve=await page.evaluate(async requestUrl=>(await fetch(requestUrl)).json(),url+"/api/curve");
  ...
  assert.equal((await page.evaluate(async requestUrl=>(await fetch(requestUrl)).json(),url+"/api/dashboard")).requested_duty,60);
  ...
  const fresh=await page.evaluate(async requestUrl=>(await fetch(requestUrl)).json(),url+"/api/dashboard");
